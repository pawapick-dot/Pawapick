            ...
            {transactions.length === 0 ? (
              ...
              transactions.map((tx: any) => (
                <div key={tx.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
                  ...
                  <div className="text-right">
                     <p className={`font-bold text-sm ${tx.type === "deposit" || tx.type === "game_win" ? "text-emerald-600" : "text-slate-900"}`}>
                       {tx.type === "deposit" || tx.type === "game_win" ? "+" : "-"}{tx.amount ? tx.amount.toLocaleString() : "0"} <span className="text-[10px] text-slate-400">UGX</span>
                     </p>
                     <span className={`text-[10px] font-bold uppercase tracking-widest ${tx.status === 'completed' || tx.status === 'Completed' ? 'text-emerald-500' : 'text-amber-500'}`}>{tx.status}</span>
                  </div>
                </div>
              ))
            )}
